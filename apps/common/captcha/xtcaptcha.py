...
from apps.common.memcached import getmem,setmem
import os
...
class Captcha(object):
    # 把一些常量抽取成类属性
    #字体的位置
    font_path = os.path.join(os.getcwd(),"apps" + os.sep + "common" + os.sep + "captcha" + os.sep + "Courgette-Regular.ttf")
    print(font_path)
    #生成几位数的验证码
    number = 4
    #生成验证码图片的宽度和高度
    ...
    def check_captcha(cls,captcha):
        ...
        if getmem(captcha_lower):
            #delete(captcha_lower)
            return True
        else:
            return False